...
//	int index = blockIdx.x * blockDim.x + threadIdx.x;
	CUDA_KERNEL_LOOP(index, count){
		...
		local_patch_data[(channel * crop_height + height_out) * crop_width + width_out] = src[(channel * src_h + (height_out + h_off)) * src_w + width_out + w_off];
	}
}
template <typename Dtype>
void LocalConvolutionLayer<Dtype>::crop_loc_patch_gpu(const Dtype *src, int src_w, int src_h, int src_c, int crop_width, int crop_height, int w_off, int h_off, Dtype *local_patch_data)
{
	...
	crop_loc_patch_kernel<Dtype><<<CAFFE_GET_BLOCKS(num_kernels)
...
/*
template<typename Dtype>
__global__ void realign_loc_conv_result_kernel(int count, const Dtype *local_conv_data
																	...
																	, Dtype *dst_data)
{
	int loc_out_spatial_dim = loc_out_h * loc_out_w;
	int loc_num = loc_num_h * loc_num_w;
	int loc_out_step = loc_out_spatial_dim * num_out;
	CUDA_KERNEL_LOOP(index, count){
		int o_c = index / loc_out_spatial_dim;
		int offset = index % loc_out_spatial_dim;
		int loc_h = offset / loc_out_w;
		int loc_w = offset % loc_out_w;
		int loc_offset = o_c * loc_out_spatial_dim + loc_h * loc_out_w + loc_w;
		int dst_c_offset = o_c * dst_h * dst_w;
		for (int i = 0; i < loc_num; ++i){
			int loc_idx_h = i / loc_num_w;
			int loc_idx_w = i % loc_num_w;
			int dst_idx = dst_c_offset + (loc_idx_h * loc_out_h + loc_h) * dst_w + loc_idx_w * loc_out_w + loc_w;
			int src_idx = i * loc_out_step + loc_offset;
	...
	const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom)
{
	...
	Dtype *bias_diff = (this->bias_term_ ? this->blobs_[1]->mutable_gpu_diff() : NULL);
	...
	const Blob<int> *idx_to_off = &this->loc_idx_to_offset_;
	const int *idx_to_off_data = idx_to_off->cpu_data();
	const int *idx_to_off_gpu_data = idx_to_off->gpu_data();
	
	Dtype *loc_bottom_data_buffer = loc_bottom_buffer_.mutable_gpu_data();
	Dtype *loc_bottom_diff_buffer = loc_bottom_buffer_.mutable_gpu_diff();
	Dtype* loc_top_data_buffer = loc_top_buffer_.mutable_gpu_data();
	Dtype* loc_top_diff_buffer = loc_top_buffer_.mutable_gpu_diff();
	...
	for (int i = 0; i < top.size(); ++i)
	{
		...
		int bottom_h = bottom[i]->height();
		int bottom_c = bottom[i]->channels();
        int conv_out_h = this->output_shape_[0];
        int conv_out_w = this->output_shape_[1];
        int conv_in_h = this->conv_input_shape_.cpu_data()[1];
        int conv_in_w = this->conv_input_shape_.cpu_data()[2];
		for (int n = 0; n < top[i]->num(); n++)
		{
			...
			for (int lh = 0; lh < local_region_num_h_; lh++)
			{
				for (int lw = 0; lw < local_region_num_w_; lw++)
				{
					...
					crop_loc_patch_gpu(single_top_diff, top_w, top_h, top_c
                                      , conv_out_w
                                      , conv_out_h
                                      , lw * conv_out_w
                                      , lh * conv_out_h
					...
					if (this->bias_term_ && this->param_propagate_down_[1]) {
						this->backward_gpu_bias(loc_bias_diff, loc_top_diff);
					}
					...
					if (this->param_propagate_down_[0] || propagate_down[i]) {
						...
						if (propagate_down[i]) {
							this->backward_gpu_gemm(loc_top_diff, loc_weight, loc_bottom_diff);
						}
					}
				}//for (int lw = 0; lw < local_region_num_w_; lw++)   
			}//for (int lh = 0; lh< local_region_num_h_; lh++) 
