...
void LocalConvolutionLayer<Dtype>::compute_output_shape() {
	...
	const int* dilation_data = this->dilation_.cpu_data(); //not use dilation here in fact
	this->output_shape_.clear();
	for (int i = 0; i < this->num_spatial_axes_; ++i) {
		// i + 1 to skip channel axis
		const int *input_dim = this->conv_input_shape_.cpu_data();
		const int kernel_extent = dilation_data[i] * (kernel_shape_data[i] - 1) + 1;
		const int output_dim = (input_dim[i + 1] + 2 * pad_data[i] - kernel_extent)
			/ stride_data[i] + 1;
...
void LocalConvolutionLayer<Dtype>::init_local_offset(){
	...
	for (h = 0; h < this->local_region_num_h_ / 2; ++h){
		...
		for (w = 0; w < this->local_region_num_w_ / 2; ++w){
			...
			idx_to_off_data[idx_to_off.offset(h, this->local_region_num_w_ - 1 - w, 0, 0)] = offset_h;
			idx_to_off_data[idx_to_off.offset(h, this->local_region_num_w_ - 1 - w, 1, 0)] = symmetry_offset_w;
	...
	for (int n = 0; n < num_output; ++n){
		...
		for (int h = 0; h < top_height; ++h){
			for (int w = 0; w < top_width; ++w){
				...
				int loc_offset = (loc_idx_h * local_region_num_w + loc_idx_w) * loc_conv_res_size;
...
template <typename Dtype>
void LocalConvolutionLayer<Dtype>::realign_bottom_diff_cpu(const Dtype *loc_bottom_diff_buffer, Dtype *bottom_diff)
{
...
void LocalConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (loc_conv_param.has_local_region_number()){
	  local_region_num_w_ = local_region_num_h_ = loc_conv_param.local_region_number();
  }else{
  ...
  if (loc_conv_param.has_local_region_ratio()){
	  local_region_ratio_w_ = local_region_ratio_h_ = loc_conv_param.local_region_ratio();
  }
  else{
  ...
  if (loc_conv_param.has_local_region_step()){
	  local_region_step_w_ = local_region_step_h_ = loc_conv_param.local_region_step();
  }
  else{
  ...
  this->is_1x1_ = kernel_shape_data[0] == 1 && kernel_shape_data[1] == 1
	  && stride_data[0] == 1 && stride_data[1] == 1 && pad_data[0] == 0 && pad_data[1] == 0;
  ...
  this->L_ = this->local_region_num_w_ * this->local_region_num_h_; //number of local regions
...
void LocalConvolutionLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  conv_input_shape_data[1] = static_cast<int>(bottom_height_ * local_region_ratio_h_);
  conv_input_shape_data[2] = static_cast<int>(bottom_width_ * local_region_ratio_w_);

  // Shape the tops.
  this->bottom_shape_ = &bottom[0]->shape();
  compute_output_shape();
  this->top_height_ = this->output_shape_[0] * this->local_region_num_h_;
  ...
  if (this->bias_term_) {    
	  vector<int> bias_multiplier_shape(1, this->output_shape_[0] * this->output_shape_[1]);
  ...
  //create map of local region index to local region offset, the local regions' offsets are central symmetry
//  int h, w, offset_w, symmetry_offset_w, offset_h, symmetry_offset_h;
  init_local_offset();
  

  loc_bottom_buffer_.Reshape(this->L_, conv_input_shape_data[0], conv_input_shape_data[1], conv_input_shape_data[2]);
  loc_top_buffer_.Reshape(this->L_, this->conv_out_channels_, this->output_shape_[0], this->output_shape_[1]);
...
//  LOG(INFO) << "Init begins";
  ...
  Dtype *bias_diff = (this->bias_term_ ? this->blobs_[1]->mutable_cpu_diff() : NULL);
  ...
  Dtype *loc_bottom_data_buffer = loc_bottom_buffer_.mutable_cpu_data();
  Dtype *loc_bottom_diff_buffer = loc_bottom_buffer_.mutable_cpu_diff();
  Dtype* loc_top_data_buffer = loc_top_buffer_.mutable_cpu_data();
  Dtype* loc_top_diff_buffer = loc_top_buffer_.mutable_cpu_diff();
