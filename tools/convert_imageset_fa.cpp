...
//DEFINE_int32(label_num, 1, "The number of classifications ")
...
int main(int argc, char** argv) {
  ...
  string filename;
 // int label_age, label_gender;
 // float expression_distribution[8];
 // float face_attributes[10];
  std::vector<float> label(40, -2);
  while (infile >> filename >> label[0] >> label[1] >> label[2] >> label[3] >> label[4] >> label[5] >> label[6] >> label[7] >> label[8] >> label[9] 
	...
	>> label[30] >> label[31] >> label[32] >> label[33] >> label[34] >> label[35] >> label[36] >> label[37] >> label[38] >> label[39] )
  {
	for(std::vector<float>::iterator itor = label.begin(); itor != label.end(); itor++)
	{
		*itor = (*itor) == -1 ? 0 : 1;
	}
